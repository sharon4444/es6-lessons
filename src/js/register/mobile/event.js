...
import Slider from '../../common/utils.js'
...
export default (conf) => {
  ...
  const slider = new Slider({
    ...
    success: async ($wrapper, $text, offsetSrr) => {
      const offsetMsg = offsetArr.join(':')
      let data = await fetchPost('/getMobileVerifyToken', {offsetArr: offsetArr})
      if (data.code === 200) {
  ...
  $verifyBtn.onclick = async () => {
    let checkResult = check($verifyForm)
